...
const animationSettings = {
    ...
    delayFactor: 0.14  // Delay between each item's animation
};
...
document.addEventListener('click', event => {
    ...
    .fromTo(allItems, { // Initial animation state
        ...
        'clip-path': 'polygon(45% 50%, 55% 50%, 50% 50%, 50% 50%)' // CSS clip-path shape
    ...
    .to(contentActive.DOM.el, {
        startAt: {scale: 1},
        scale: 1.4,
        onComplete: () => gsap.to(contentActive.DOM.el, {scale: 1})
    }, 0)
    .add((() => toggleContent()))
    ...
    .to(lastItem, { // Animation for the last item
        duration: 1,
        ease: 'expo', 
        'clip-path': 'polygon(0% 100%, 100% 100%, 100% 100%, 0% 100%)', // Animating clip-path to different shape
